...
export class DataProcessorService {
  ...
  private static mapRowToTransaction(headers: string[], values: string[], rowIndex: number): Transaction | null {
    ...
    const amountStr = getColumnValue(["amount", "transaction amount", "debit", "credit"])
    const description = getColumnValue(["description", "memo", "transaction description", "details"])

    if (!dateStr || !amountStr || !description) {
      console.warn(`Row ${rowIndex + 1}: Missing required fields (date, amount, description)`)
      return null
    }
    ...
    try {
      ...
      console.warn(`Row ${rowIndex + 1}: Invalid amount format: ${amountStr}`)
      return null
    }
